    ...
    formatters: {
        "commands": function(column, row) {  
            if(row.response == 'Pending') {
                return "<button type='button' class='btn btn-danger btn-xs Apply' data-row-id='"+row.id+"'>Apply</button>";
            } else {
//               return "<button type='button' class='btn btn-success btn-xs Active' data-row-id='"+row.id+"'>Active</button>";
    ...
    grid.find(".Apply").on("click", function(e)
    {
        ...
        var response = applyJob(clickedId, 2);
    ...
    function applyJob(clickedId, action) {
         $.ajax({  
            url:"applyjob.php",  
            method:"POST",  
